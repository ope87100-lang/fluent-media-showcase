...
# Unified vCard containing Organization, Founder, Website, and BOTH Phone numbers
MASTER_DATA = """BEGIN:VCARD
VERSION:3.0
FN:Fluent Media Marketing
ORG:Fluent Media Marketing
TITLE:Zubair Jamil (Founder & Head Strategist)
TEL;TYPE=CELL,PREF:+923294357248
TEL;TYPE=WORK:+923184628170
URL:https://fluent-media-marketing-bg11.vercel.app/
ADR;TYPE=WORK:;;Lahore;;;Pakistan
NOTE:Premier Digital Performance & Meta Ads Agency
END:VCARD"""
...
def create_master_framed(qr_img):
    ...
    subtitle = "SCAN TO SAVE CONTACTS & OPEN WEBSITE"
    ...
    # Line 2: Website URL
    url_text = "WEBSITE:  https://fluent-media-marketing-bg11.vercel.app/"
    ...
    # Line 3: Quick Helper Tip
    tip_text = "Scan with any iPhone or Android camera to instantly save Zubair Jamil & Fluent Media"
...
def main():
    print("Re-generating ONE Master QR Code with clean typography...")
    
    master_qr = create_master_qr()
    path_pure = os.path.join(OUTPUT_DIR, "master_qr_code.png")
    master_qr.save(path_pure, "PNG")
    master_qr.save(os.path.join(ARTIFACT_DIR, "master_qr_code.png"), "PNG")
    
    framed = create_master_framed(master_qr)
    path_framed = os.path.join(OUTPUT_DIR, "master_qr_code_framed.png")
    framed.save(path_framed, "PNG")
    framed.save(os.path.join(ARTIFACT_DIR, "master_qr_code_framed.png"), "PNG")
    ...
    qr_svg.add_data(MASTER_DATA)
    qr_svg.make(fit=True)
    svg_img = qr_svg.make_image(fill_color="#0f172a", back_color="#ffffff")
    path_svg = os.path.join(OUTPUT_DIR, "master_qr_code.svg")
    svg_img.save(path_svg)
    svg_img.save(os.path.join(ARTIFACT_DIR, "master_qr_code.svg"))
    
    print("Master QR Code re-generated successfully!")
